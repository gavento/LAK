\df {\it Skalárním součinem} vektorů $x=(x_1,\dots,x_n)^T$ a $y=(y_1,\dots,y_n)^T$ z~vektorového prostoru $\Ft^n$ rozumíme číslo $\langle x,y\rangle=\sum_{i=1}^nx_iy_i\in\Ft$ (případně $\langle x,y\rangle=\sum_{i=1}^nx_i\overline{y_i}$ pro $\Ft=\Compl$).
